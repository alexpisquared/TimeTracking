...
public partial class FromTillCtgrTaskNote : AAV.WPF.Base.WindowBase
{
  ...
  DateTime firstUn_OrLastInvoced()
  {
    try
    {
      ...
      return iv != null ? iv.PeriodUpTo.AddDays(1) : App.AppStartAt;
    }
  ...
  async Task<bool> tryLoadCreate(bool createDb)
  {
    ...
    catch (EntityException eex)
    {
      ...
      return MessageBox.Show(eex.ToString(), "No Database Found - OK to generate new database?", MessageBoxButton.YesNo, MessageBoxImage.Question) switch
      {
        MessageBoxResult.Yes => await tryLoadCreate(true),
  ...
  void onAdd8hrDays(object s, RoutedEventArgs e)
  {
    ...
    _ = Task<List<TimePerDay>>.Factory.StartNew(() => get8hrTimes(ctg, tps, bgn, end, dsh, WorkedHrs.DailyHours)).ContinueWith(_ =>
    {
      try
      {
  ...
  void onInfer(object s, RoutedEventArgs e)
  {
    ...
    _ = Task<List<TimePerDay>>.Factory.StartNew(() => getNewTimes(ctg, tps, bgn, end)).ContinueWith(_ =>
    {
  ...
  void btnUnderConst2Clk(object s, RoutedEventArgs e)
  {
    ...
    {
      if (c is not TimePerDay) continue;
