...
public static class Emailer
{
  public static async Task<bool> SmtpSend(string fromEmail, string trgEmail, string msgSubject, string msgBody, string[]? attachedFilenames = null, string? signatureImage = null)
  {
    try
    {
      trgEmail = "pigida@gmail.com"; //todo: (ap: 2022-04-21)
...
#else
        //tu: add to App.cfg: <system.net><mailSettings><smtp deliveryMethod="Network" from="test@foo.com"><!--userName="pigida@aei.ca" password=""--><!--port="25"--><network host="mail.aei.ca" defaultCredentials="true"/></smtp></mailSettings></system.net>
        var mailClient = new SmtpClient
        {
          UseDefaultCredentials = true,
