...
namespace TimeTracker.Common
{
	public class TimeTrackDbCtx_Code1st_DbInitializer : DropCreateDatabaseIfModelChanges<A0DbContext>
	{
		...
		public static void DropCreateDb()
		{
			...
			catch (Exception ex) { MessageBox.Show(ex.ToString(), "InvalidOperationException has been thrown", MessageBoxButton.OK, MessageBoxImage.Error); }
		}
		...
		protected override void Seed(A0DbContext x)
		{
			try
			{
				...
				x.lkuJobCategories.Add(new lkuJobCategory { Id = "dev", Name = "Development", Description = "Software Development Services" });
				x.lkuJobCategories.Add(new lkuJobCategory { Id = "mtg", Name = "Meetings", Description = "Meetings" });
				x.lkuJobCategories.Add(new lkuJobCategory { Id = "off", Name = "Time off", Description = "Time off" });
				x.lkuJobCategories.Add(new lkuJobCategory { Id = "vac", Name = "Vacation", Description = "Vacation" });
